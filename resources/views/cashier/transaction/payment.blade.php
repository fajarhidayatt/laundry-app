...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-md-12 col-lg-12 col-sm-12 col-xs-12">
            <div class="white-box">
                <form method="post" action="/cashier/transaction/{{ $transaction->id }}">
                    @csrf
                    @method('PUT')
                    <div class="form-group">
                        <label for="invoice">Kode Invoice</label>
                        <input type="text" name="invoice" id="invoice" class="form-control" value="{{ $transaction->invoice }}" readonly>
                    </div>
                    <div class="form-group">
                        <label for="member_name">Nama Member</label>
                        <input type="text" name="member_name" id="member_name" class="form-control" value="{{ $transaction->member->name }}" readonly>
                    </div>
                    <div class="form-group">
                        <label for="total_price">Total Yang Harus Di Bayar</label>
                        <input type="text" name="total_price" id="total_price" class="form-control" value="{{ $transaction->detailTransaction->total_price }}" readonly style="display: none;">
